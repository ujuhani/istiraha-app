...
export default function App(){
  ...
  const calcBudget=async()=>{
    ...
    if(existing){showToast(`⚠️ موازنة ${budgetMonth} ${budgetYear} نُفِّذت بالفعل في ${existing.date}`,"warn");return;}
    ...
    const memberBreakdown=plusMembers.map(m=>{
      const memberDebtTotal=activeDebts.filter(d=>d.memberName===m.name).reduce((s,d)=>s+d.remaining,0);
      const deducted=Math.min(equalShare,memberDebtTotal);
      const stillOwes=Math.max(0,equalShare-memberDebtTotal);
      return{...m,equalShare,memberDebtTotal,deducted,stillOwes};
    });
    setBudgetResult({month:budgetMonth,year:budgetYear,mInc,mExp,bal,deficit,equalShare,memberBreakdown,isNew:true});
    setModal("budget");
  };
      ...
      for(const m of budgetResult.memberBreakdown){
        if(m.deducted>0){
          ...
          for(const d of memberDebts){
            if(toDeduct<=0)break;
            const deductFromThis=Math.min(toDeduct,d.remaining);
            await updateDoc(doc(db,"debts",d.id),{remaining:d.remaining-deductFromThis});
            toDeduct-=deductFromThis;
          }
        }
      }
      await addDoc(collection(db,"balances"),{month:budgetResult.month,year:budgetResult.year,date:todayStr(),mInc:budgetResult.mInc,mExp:budgetResult.mExp,deficit:budgetResult.deficit,equalShare:budgetResult.equalShare,memberBreakdown:budgetResult.memberBreakdown,debtSnapshot,createdAt:Date.now()});
      showToast(`✅ تمت موازنة ${budgetResult.month} ${budgetResult.year}`);
      setModal(null);setBudgetResult(null);
    }catch(e){showToast("حدث خطأ، حاول مجدداً","error");}
    setProcessing(false);
  };
  const resetLastBalance=async()=>{
    if(!lastBalance){showToast("لا توجد موازنة لإعادتها","warn");return;}
    setProcessing(true);
    try{
  ...
  return(<div style={{minHeight:"100vh",background:"#EEF2F7",fontFamily:T.font,direction:"rtl",padding:"18px 14px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        ...
        <div style={{overflowX:"auto",maxHeight:300,overflowY:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              ...
              {[...income].sort((a,b)=>b.createdAt-a.createdAt).map(r=>(<tr key={r.id}><td style={TD}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:7,height:7,borderRadius:"50%",flexShrink:0,background:members.find(m=>m.name===r.memberName)?.type==="plus"?T.purple:T.gray}}/>{r.memberName}</div></td><td style={TD}>{r.month}</td><td style={{...TD,color:T.gray,fontSize:11}}>{r.year}</td><td style={{...TD,color:T.green,fontWeight:700}}>{fmt(r.amount)}</td><td style={{...TD,padding:"5px 8px"}}><button onClick={()=>del("income",r.id)} style={{background:T.redLight,border:"none",borderRadius:6,padding:"4px 6px",cursor:"pointer",color:T.red,display:"flex"}}><IcTrash/></button></td></tr>))}
            </tbody>
          </table>
        </div>
      </div>
      <div style={{background:T.white,borderRadius:16,boxShadow:"0 2px 10px rgba(0,0,0,.07)",overflow:"hidden"}}>
        <div style={{background:`linear-gradient(90deg,${T.redDark}ee,${T.redDark}bb)`,padding:"13px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <Btn size="sm" color={T.white} textColor={T.redDark} onClick={()=>{setExpF({desc:"",cat:"",memberName:"",date:todayStr(),amount:""});setExpErr({});setModal("expense");}}><IcPlus/> إضافة</Btn>
        </div>
        <div style={{overflowX:"auto",maxHeight:300,overflowY:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead style={{position:"sticky",top:0}}><tr>{["البيان","التصنيف","العضو","المبلغ",""].map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead>
            <tbody>
              {expenses.length===0&&<tr><td colSpan={5} style={{...TD,textAlign:"center",color:T.gray,padding:20}}>لا توجد مصاريف</td></tr>}
              {[...expenses].sort((a,b)=>b.createdAt-a.createdAt).map(r=>(<tr key={r.id}><td style={TD}>{r.desc}</td><td style={TD}><span className="badge" style={{background:T.slate2,color:T.gray}}>{r.cat}</span></td><td style={TD}>{r.memberName?<span style={{color:T.purple,fontWeight:600}}>{r.memberName}</span>:<span style={{color:T.gray,fontSize:11}}>—</span>}</td><td style={{...TD,color:T.red,fontWeight:700}}>{fmt(r.amount)}</td><td style={{...TD,padding:"5px 8px"}}><button onClick={()=>del("expenses",r.id)} style={{background:T.redLight,border:"none",borderRadius:6,padding:"4px 6px",cursor:"pointer",color:T.red,display:"flex"}}><IcTrash/></button></td></tr>))}
            </tbody>
          </table>
        </div>
      </div>
    ...
    {modal==="budget"&&budgetResult&&(<Modal title={`📊 موازنة ${budgetResult.month} ${budgetResult.year}`} onClose={()=>setModal(null)} width={500}>
      ...
      {budgetResult.deficit>0?(<>
        <div style={{background:T.amberLight,border:`1px solid ${T.amber}44`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:13,color:"#92400E"}}>⚠️ عجز <strong>{fmt(budgetResult.deficit)}</strong> — نصيب كل عضو+ <strong>{fmt(budgetResult.equalShare)}</strong></div>
        <div style={{fontSize:13,fontWeight:700,color:T.navy,marginBottom:8}}>توزيع العجز على الأعضاء+:</div>
        <div style={{borderRadius:12,overflow:"hidden",border:`1px solid ${T.slate3}`,marginBottom:14}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr>{["العضو","نصيبه","دينه على الاستراحة","يُخصم","المتبقي عليه"].map(h=><th key={h} style={{...TH,background:T.navy,fontSize:10,padding:"8px"}}>{h}</th>)}</tr></thead>
            <tbody>{budgetResult.memberBreakdown.map(m=>(<tr key={m.id}>
              <td style={TD}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:26,height:26,borderRadius:"50%",background:T.purple,color:T.white,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,flexShrink:0}}>{m.name[0]}</span>{m.name}</div></td>
              <td style={{...TD,color:T.orange,fontWeight:700}}>{fmt(m.equalShare)}</td>
              <td style={{...TD,color:T.purple,fontWeight:600}}>{fmt(m.memberDebtTotal)}</td>
              <td style={{...TD,color:T.green,fontWeight:700}}>{fmt(m.deducted)}</td>
              <td style={TD}>{m.stillOwes<=0?<span className="badge" style={{background:T.greenLight,color:T.green}}>✅ مغطّى</span>:<span style={{color:T.red,fontWeight:700}}>{fmt(m.stillOwes)}</span>}</td>
            </tr>))}</tbody>
          </table>
        </div>
      ...
      {budgetResult.isNew&&<div style={{background:T.blueLight,borderRadius:10,padding:"10px 14px",fontSize:12,color:T.navy,marginBottom:14}}>📌 سيتم خصم مبلغ العجز من ديون كل عضو+ على الاستراحة عند التأكيد</div>}
